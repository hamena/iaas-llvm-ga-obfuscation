            ...
            for i in range(parents[0].number_of_variables):
                ...
                if random.random() <= 0.5:
                    if abs(value_x1 - value_x2) > self.__EPS:
                        ...
                        if random.random() <= 0.5:
                            offspring[0].variables[i] = round(c2)
                            offspring[1].variables[i] = round(c1)
                        else:
                            offspring[0].variables[i] = round(c1)
                            offspring[1].variables[i] = round(c2)
                    else:
                        offspring[0].variables[i] = round(value_x1)
                        offspring[1].variables[i] = round(value_x2)
                else:
                    offspring[0].variables[i] = round(value_x1)
                    offspring[1].variables[i] = round(value_x2)
        return offspring
